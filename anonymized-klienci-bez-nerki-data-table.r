...
# filtering out orders containing 'nerka' of any kind
dt.filtered <- dt.processed %>%
  filter(!grepl('nerka', product_name_5, ignore.case = TRUE)) %>%
  filter(!grepl('nerka', product_name_4, ignore.case = TRUE)) %>%
  filter(!grepl('nerka', product_name_3, ignore.case = TRUE)) %>%
  filter(!grepl('nerka', product_name_2, ignore.case = TRUE)) %>%
  filter(!grepl('nerka', product_name_1, ignore.case = TRUE)) # 6,231 entries
